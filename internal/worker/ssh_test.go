...
func TestParseInspection(t *testing.T) {
	input := `__CF_HOSTNAME__worker-host
__CF_USER__fleet-user
__CF_TAGS__
...
__CF_PS_END__
`
	inspection, err := parseInspection("worker-1", []byte(input))
	if err != nil {
		t.Fatalf("parseInspection() error = %v", err)
	}
	if inspection.Hostname != "worker-host" || inspection.User != "fleet-user" {
