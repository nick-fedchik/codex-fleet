...
Для master він виглядає так само, як Jetson: окремий Linux-користувач, SSH-ключ
і Ollama.
...
- ім'я вузла, наприклад `aorus5-wsl2`;
...
Перезавантажити Windows. Під час першого запуску Ubuntu створити Linux-користувача
`codex` і пароль. Якщо WSL2 та Ubuntu вже встановлені, цей крок пропустити.
...
У Ubuntu, під користувачем `codex`:
...
```bash
ssh aorus5-wsl2 'hostname; id -un; ollama list; ollama ps'
```

Очікуваний результат: ім'я Windows/WSL-хоста, користувач `codex` і список моделей.
...
- після цього користувач працює як звичайний Linux-користувач `codex`;
